...
// ConversionError defines an error that describes failure of type convertion.
// It might be return from ConvertValue when type convertion failed.
type ConversionError struct {
	err          error
	Val          string
	ExpectedType descriptor.FieldDescriptorProto_Type
}

func (e *ConversionError) Error() string {
	return fmt.Sprintf("failed to convert an inputted value '%s' to type %s: %s", e.Val, e.ExpectedType, e.err)
}

// ConvertValue holds value and error of conversion
// each cast (Parse*) returns falsy value when failed to parse argument
func ConvertValue2(pv string, field *desc.FieldDescriptor) (interface{}, error) {
	t := descriptor.FieldDescriptorProto_Type(descriptor.FieldDescriptorProto_Type_value[field.GetType().String()])
	...
	if err != nil {
		return nil, &ConversionError{err: err, Val: pv, ExpectedType: t}
	}
	return v, nil
}
...
// Also ToEntitiesFromServiceDescriptors returns messages which are containing to request/response message fields or itself.
