...
func (c *Command) parseFlags(args []string) *options {
	...
	f.StringVar(&opts.cacert, "cacert", "", "the CA certificate file for mutual TLS auth. it must be provided with --cert and --certkey.")
	f.StringVar(&opts.cert, "cert", "", "the certificate file for mutual TLS auth. it must be provided with --cacert and --certkey.")
	f.StringVar(&opts.certKey, "certkey", "", "the private key file. it must be provided with --cacert and --cert.")
